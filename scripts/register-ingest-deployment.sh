  ...
  --overrides="$(cat <<EOF
{
  "spec": {
    ...
    "containers": [{
      ...
      "command": ["sh", "-c", "prefect work-pool inspect kubernetes >/dev/null 2>&1 || prefect work-pool create kubernetes --type kubernetes; prefect deploy --name ingest-document --pool kubernetes --job-variable image=${WORKERS_IMAGE} && echo DEPLOY_OK && sleep 120"]
    }]
  }
}
EOF
